...
VALUE5=$(pmset -g | grep "standbydelay" | cut -d " " -f 8)
VALUE6=$(pmset -g | grep "autopoweroff123 " | cut -d " " -f 11)

if [ "$VALUE1" = "1" ] && [ "$VALUE2" = "25" ] && [ "$VALUE3" = "0" ] && [ "$VALUE4" = "0" ] && [ "$VALUE5" = "0" ] && [ "$VALUE6" = "0" ] || [ -z "$VALUE6" ]; then
    echo "1"
else
    echo "0"
fi
